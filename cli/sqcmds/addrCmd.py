...
class addrCmd(SQCommand):
    ...
    @argument('match', description='type of match: exact or subnet',
    ...
    def show(self, address: str = '', match='subnet'):
        ...
        df = self.addrobj.get(hostname=self.hostname,
                              columns=self.columns, address=address,
                              datacenter=self.datacenter, match=match)
