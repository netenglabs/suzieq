...
class SQSparkEngine(SQEngine):
    ...
    def get_table_df(self, cfg, schemas, **kwargs):
        ...
        qstr = self.build_sql_str(table, start_time, end_time, view,
                                  sort_fields, sch, **kwargs)
        if not qstr:
            return None

        df = self.get_query_df(qstr, cfg, schemas,
    ...
    def build_sql_str(self, table: str, start_time: str, end_time: str,
                      view: str, sort_fields: list, sch, **kwargs):
    ...
    def get_spark_code(self, qstr: str, cfg, schemas, start: str = '',
                       end: str = '', view: str = 'latest') -> str:
        ...
        if counter or view == 'all':
            ...
            timestr = '('
            if start:
                ssecs = int(datetime.strptime(start, '%Y-%m-%d %H:%M:%S').
                            strftime('%s'))*1000
                timestr += "timestamp > {} ".format(ssecs)
            if end:
                esecs = int(datetime.strptime(end, '%Y-%m-%d %H:%M:%S').
                            strftime('%s'))*1000
                if timestr != '(':
                    timestr += (
    ...
    def get_query_df(self, query_string: str, cfg, schemas,
        ...
        code = self.get_spark_code(query_string, cfg, schemas, start_time,
                                   end_time, view)
