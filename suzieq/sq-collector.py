...
def start_new_ssquery(spark, topic, kafka_servers, pqdir, schema):
    ...
    out = (
        ...
        .option("checkpointLocation", "{}/_checkpoints/{}".format(pqdir, topic))
...
def _main(userargs):
    ...
    spark = SparkSession.builder.appName("suzieq-pq").getOrCreate()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="/tmp/suzieq/",
        help="Absolute path to directory to store parquet " "output in",
    )
    parser.add_argument(
        "-s",
        "--service-only",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Absolute path to directory with schema " "definition for services",
    )
    ...
    if userargs.foreground:
        ...
        with daemon.DaemonContext(pidfile=pidfile.TimeoutPIDLockFile(PID_FILE)):
            _main(userargs)
