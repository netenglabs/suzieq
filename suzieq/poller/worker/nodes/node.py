...
class Node:
    ...
    async def initialize(self, **kwargs) -> TNode:
        ...
        self._max_retries_on_auth_fail = (kwargs.get("retries_on_auth_fail") or 0) + 1
        ...
        if jump_host:
            ...
            if pvtkey_file:
                self.jump_host_key = self._decrypt_pvtkey(pvtkey_file, passphrase)
                if not self.jump_host_key:
                    raise SqPollerConfError("Unable to read private key file" f" at {pvtkey_file}")
        else:
            self.jump_host = None
            self.jump_host_key = None
        ...
        if pvtkey_file:
            ...
            if not self.pvtkey:
                self.logger.error("ERROR: Falling back to password for " f"{self.address}:{self.port}")
                self.pvtkey = None
        else:
            self.pvtkey = None
        ...
        if devtype:
            ...
            self.logger.warning(f"{devtype} supplied for {self.address}:{self.port}")
        ...
        if not devtype:
            if self.transport != "ssh":
                self.logger.error("devtype MUST be specified in inventory file if transport" f"is not ssh for {self.address}")
                return self
    ...
    def _decrypt_pvtkey(self, pvtkey_file: str, passphrase: str) -> str:
        ...
        if pvtkey_file:
            try:
                keydata = asyncssh.public_key.read_private_key(pvtkey_file, passphrase)
            except Exception as e:  # pylint: disable=broad-except
                self.logger.error(f"ERROR: Unable to read private key file {pvtkey_file}" f"for jump host due to {e}")
    ...
    async def _terminate(self):
        self.logger.warning(f"Node: {self.hostname} received signal to terminate")
    ...
    def _create_error(self, cmd) -> dict:
        ...
        elif isinstance(self.current_exception, asyncssh.misc.PermissionDenied):
            status = HTTPStatus.FORBIDDEN
    ...
    async def _parse_device_type_hostname(self, output, _) -> None:
        ...
        if output[0]["status"] == 0:
            ...
            if "Arista" in data or "vEOS" in data:
                ...
                self.logger.info(f"{self.address}: Got unrecognized device show version: " f"{data}")
        ...
        if devtype == "unsupported":
            if not self.current_exception:
                self.logger.info(f"Unable to determine devtype for " f"{self.address}:{self.port}")
                self._set_devtype(devtype, version_str)
                self._set_hostname(self.address)
                await self._close_connection()
                self.current_exception = UnknownDevtypeError()
        elif devtype:
            self.logger.warning(f"Detected {devtype} for {self.address}:{self.port}," f" {hostname}")
            self._set_devtype(devtype, version_str)
    ...
    async def _detect_node_type(self):
        ...
        if self.transport in ["ssh", "local"]:
            try:
                ...
                self.logger.exception(f"{self.address}:{self.port}: Node " "discovery failed due to exception")
            ...
            if not self.devtype:
                self.logger.debug(f"No devtype for {self.hostname} {self.current_exception}")
            ...
            self.logger.error(f"Non-SSH transport node {self.address}:{self.port} " "has no devtype specified. Node will not be polled")
    ...
    async def _get_device_type_hostname(self):
        ...
        await self._exec_cmd(self._parse_device_type_hostname, ["show version", "cat /etc/os-release && hostname"], None, "text", only_one=True)
    ...
    def _set_devtype(self, devtype: str, version_str: str) -> None:
        ...
        if devtype not in known_devtypes():
            self.logger.error(f"An unknown devtype {devtype} is being added." f" This will cause problems. " f"Node {self.address}:{self.port}")
            raise ValueError
    ...
    async def _init_jump_host_connection(self, options: asyncssh.SSHClientConnectionOptions) -> None:
        ...
        if self.jump_host_key:
            ...
            if self.ignore_known_hosts:
                jump_host_options = asyncssh.SSHClientConnectionOptions(options=jump_host_options, known_hosts=None)
            if self.ssh_config_file:
                jump_host_options = asyncssh.SSHClientConnectionOptions(options=jump_host_options, config_file=[self.ssh_config_file])
        else:
            jump_host_options = options

        try:
            if self.jump_host:
                self.logger.info("Using jump host: %s, with username: %s, and port: %s", self.jump_host, self.jump_user, self.jump_port)
                self._tunnel = await asyncssh.connect(self.jump_host, port=self.jump_port, options=jump_host_options, username=self.jump_user)
                self.logger.info("Connection to jump host %s succeeded", self.jump_host)

        except Exception as e:  # pylint: disable=broad-except
            self.logger.error(f"Cannot connect to jump host: {self.jump_host} ({e})")
    ...
    async def _init_ssh(self, init_dev_data=True, use_lock=True) -> None:
        ...
        """
        if self._retry and (not self.is_connected or self._is_connecting) and time.time() > self._connect_again_at:
            someone_tried = False
            if use_lock:
            ...
            attempts = 0
            backoff_period = self.CONN_INITIAL_BACKOFF_SLOW if self.slow_host else self.CONN_INITIAL_BACKOFF
            try:
                while not self.is_connected:
                    ...
                    if self.is_connected:
                        self.logger.info("Connection succeeded via SSH for " f"{self.hostname}")
                        break
                ...
                if self.is_connected:
                    ...
                    wait_time = max(self.MIN_WAIT_TIME_CONN_FAIL, backoff_period * 3)
                    self._connect_again_at = time.time() + wait_time
                    next_time = datetime.fromtimestamp(self._connect_again_at)
                    logger.info(f"Connection to {self.address}:{self.port} will be " f"retried from {next_time}")
            finally:
                self._is_connecting = False
                if use_lock:
                    self.ssh_ready.release()
    ...
    async def _ssh_connect(self):
        ...
        async with self._cmd_pacer.wait():
            try:
                if self._tunnel:
                    self._conn = await self._tunnel.connect_ssh(self.address, port=self.port, username=self.username, options=options)
                else:
                    self._conn = await asyncssh.connect(self.address, username=self.username, port=self.port, options=options)
                self.logger.info(f"Connected to {self.address}:{self.port} at " f"{time.time()}")
                ...
                if isinstance(e, asyncssh.HostKeyNotVerifiable):
                    ...
                    if not self._retry:
                        error_msg += "Not retrying to avoid locking out " "user. Please restart poller with " "proper authentication."
                    self.logger.error(f"{error_msg}: {e}")

                else:
                    self.logger.error("Unable to connect to " f"{self.address}:{self.port}, {e}")
                self.current_exception = e
                await self._close_connection()
                self._conn = None
    ...
    async def _init_rest(self):
        ...
        raise NotImplementedError(f"{self.address}: REST transport is not supported")

    # pylint: disable=unused-argument
    async def _local_gather(self, service_callback: Callable, cmd_list: List[str], cb_token: RsltToken, oformat: str, timeout: int):
        ...
        for cmd in cmd_list:
            ...
            try:
                stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=self.cmd_timeout)
    ...
    def _schedule_discovery_attempt(self):
        ...
        logger.info(f"Discovery of {self.address}:{self.port} will be retried " f"from {next_time}")

    # pylint: disable=unused-argument
    async def _ssh_gather(
        self,
        service_callback: Callable,
        cmd_list: List[str],
        cb_token: RsltToken,
        oformat: str,
        timeout: int,
        ...
        if not self.is_connected or self._is_connecting:
            ...
            if not self.is_connected:
                for cmd in cmd_list:
                    self.logger.error("Unable to connect to node %s cmd %s", self.hostname, cmd)
                    result.append(self._create_error(cmd))
                await service_callback(result, cb_token)
                return
        ...
        async with self._cmd_pacer.wait(self.per_cmd_auth):
            for cmd in cmd_list:
                try:
                    ...
                    output = await asyncio.wait_for(self._conn.run(cmd), timeout=timeout)
                    if self.current_exception:
                        self.logger.info("%s recovered from previous exception", self.hostname)
                        self.current_exception = None
                    ...
                    if not isinstance(e, asyncio.TimeoutError):
                        self.logger.error("%s output for %s failed due to %s", cmd, self.hostname, e)
                        await self._close_connection()
                    else:
                        self.logger.error("%s output for %s failed due to timeout", cmd, self.hostname)

                    break

        await service_callback(result, cb_token)

    async def _exec_cmd(self, service_callback, cmd_list, cb_token, oformat="json", timeout=None, only_one=False, reconnect=True):
        ...
        """
        if self.transport == "ssh":
            await self._ssh_gather(service_callback, cmd_list, cb_token, oformat, timeout, reconnect, only_one)
        elif self.transport == "https":
            await self._rest_gather(service_callback, cmd_list, cb_token, oformat, timeout)
        elif self.transport == "local":
            await self._local_gather(service_callback, cmd_list, cb_token, oformat, timeout)
        else:
            self.logger.error("Unsupported transport %s for node %s", self.transport, self.hostname)
        return

    # pylint: disable=too-many-nested-blocks
    async def _exec_service(self, service_callback, svc_defn: dict, cb_token: RsltToken):
        ...
        if not self.devtype and self._retry and self.init_again_at < time.time():
        ...
        if not self.devtype or (self.devtype and self._discovery_lock.locked()):
        ...
        if self.devtype == "unsupported":
            ...
            result.append(self._create_result(svc_defn, 418, "No service definition"))
        ...
        if not use:
            if svc_defn.get("service") not in self.error_svcs_proc:
                result.append(self._create_result(svc_defn, HTTPStatus.NOT_FOUND, "No service definition"))
                self.error_svcs_proc.add(svc_defn.get("service"))
            return await service_callback(result, cb_token)
            ...
            if isinstance(use, list):
                ...
                for item in use:
                    if item.get("version", "") != "all":
                        ...
                        opdict = {">": operator.gt, "<": operator.lt, ">=": operator.ge, "<=": operator.le, "=": operator.eq, "!=": operator.ne}
                        op = operator.eq
                        ...
                        if op(version_parse.LegacyVersion(self.version), version_parse.LegacyVersion(os_version)):
        ...
        await self._exec_cmd(service_callback, cmdlist, cb_token, oformat=oformat, timeout=cb_token.timeout)
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        ...
        Args:
            reconnect (bool): try connection if the poller is not connected to
                the device.
        """
        raise NotImplementedError(f"{self.address}: _fetch_init_dev_data_devtype not implmeneted in " "base Node class")

    async def _parse_init_dev_data(self, output: List, cb_token: RsltToken) -> None:
        ...
        failed_commands = [out for out in output if out["status"] not in [0, 200]]
        if failed_commands:
            # Get the reason for the failure of the commands
            fail_reason = ""
            for cmd in failed_commands:
                fail_reason += f"Cmd `{cmd['cmd']}` failed with reason: " f"{cmd['data']}. "

            # Schedule a new connection attempt to retry the discovery of the
            # node
            min_wait_time = self.MIN_WAIT_TIME_DATA_INIT_FAIL_SLOW if self.slow_host else self.MIN_WAIT_TIME_DATA_INIT_FAIL
    ...
    @abstractmethod
    async def _parse_init_dev_data_devtype(self, output: List, cb_token: RsltToken) -> None:
        """Parse the version, uptime and hostname info from the output
        ...
        """
        raise NotImplementedError(f"{self.address}: parsing init base Node class")
    ...
    def _extract_nos_version(self, data: str) -> str:
        ...
        """
        raise NotImplementedError(f"{self.address}: extracting NOS in init base Node class")

    @abstractmethod
    async def _rest_gather(
        self, service_callback: Callable, cmd_list: List[str], cb_token: RsltToken, oformat: str = "json", timeout: int = None
    ):
    ...
    def post_commands(self, service_callback: Callable, svc_defn: Dict, cb_token: RsltToken):
        ...
        try:
            while True:
                while len(tasks) < self.batch_size:
                    ...
                    if request:
                        tasks.append(self._exec_service(request[0], request[1], request[2]))
                        self.logger.debug(f"Scheduling {request[2].service} for execution")
...
class EosNode(Node):
    ...
    async def _init_rest(self):
        ...
        auth = aiohttp.BasicAuth(self.username, password=self.password or "vagrant")
        url = f"https://{self.address}:{self.port}/command-api"

        try:
            async with aiohttp.ClientSession(auth=auth, timeout=self.connect_timeout, connector=aiohttp.TCPConnector(ssl=False)) as session:
                async with session.post(url, timeout=timeout) as response:
                    _ = response.status
        except Exception as e:
            self.logger.error(f"Unable to connect to {self.address}:{self.port}, " f"error: {str(e)}")
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        ...
        await self._exec_cmd(self._parse_init_dev_data, cmdlist, None, reconnect=reconnect)

    async def _ssh_gather(self, service_callback, cmd_list, cb_token, oformat, timeout, reconnect, only_one=False):
        """Use SSH to execute the commands requested
        ...
        return await super()._ssh_gather(service_callback, newcmd_list, cb_token, oformat, timeout, reconnect, only_one)

    async def _rest_gather(self, service_callback, cmd_list, cb_token, oformat="json", timeout=None):
        ...
        async with self._cmd_pacer.wait(self.per_cmd_auth):
            try:
                async with aiohttp.ClientSession(
                    auth=auth, conn_timeout=self.connect_timeout, read_timeout=timeout, connector=aiohttp.TCPConnector(ssl=False)
                ) as session:
                                ...
                                result.append(
                                    {
                                        ...
                                        "data": output[i] if isinstance(output, list) else output,
                                    }
                                )
                        else:
                            for cmd in cmd_list:
                                result.append(self._create_error(cmd))
                            self.logger.error(
                                f"{self.transport}://{self.hostname}:" f"{self.port}: Commands failed due to " f"{response.status}"
                            )
            except Exception as e:
                self.current_exception = e
                for cmd in cmd_list:
                    result.append(self._create_error(cmd))
                self.logger.error(f"{self.transport}://{self.hostname}:{self.port}: Unable " f"to communicate with node due to {str(e)}")
        ...
        if output[0]["status"] == 0 or output[0]["status"] == 200:
            if self.transport == "ssh":
                try:
                    ...
                    self.logger.error(f"nodeinit: Error decoding JSON for " f"{self.address}:{self.port}")
                    return
            else:
                data = output[0]["data"]
            ...
            if not self.version:
                self.logger.error(f"nodeinit: Error getting version for " f"{self.address}: {self.port}")
    ...
    def _extract_nos_version(self, data) -> None:
        ...
        if isinstance(data, str):
            match = re.search(r"Software Image Version:\s+(\S+)", data, re.IGNORECASE)
            if match:
                self.version = match.group(1).strip()
            else:
                self.logger.warning(f"Cannot parse version from {self.address}:{self.port}")
                self.version = "all"
        else:
...
class CumulusNode(Node):
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        ...
        await self._exec_cmd(
            self._parse_init_dev_data, ["cat /proc/uptime", "hostname", "cat /etc/os-release"], None, "text", reconnect=reconnect
        )
    ...
    async def _parse_init_dev_data_devtype(self, output, _) -> None:
        ...
        if output[0]["status"] == 0:
            ...
            self.bootupTimestamp = int(int(time.time() * 1000) - float(upsecs) * 1000)
    ...
    def _extract_nos_version(self, data: str) -> None:
        ...
        if version_str:
            ...
            self.version = "all"
            self.logger.error(f"Cannot parse version from {self.address}:{self.port}")
...
class IosXRNode(Node):
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        """Fill in the boot time of the node by executing certain cmds"""
        await self._exec_cmd(self._parse_init_dev_data, ["show version", "show run hostname"], None, "text", reconnect=reconnect)
    ...
    async def _ssh_connect(self):
        ...
        if self.is_connected and not self._long_proc:
            try:
                self._long_proc = await self._conn.create_process("run tail -s 3600 -f /etc/version", stdout=DEVNULL, stderr=DEVNULL)
                self.logger.info(f"Persistent SSH present for {self.hostname}")
            except Exception as e:
                if isinstance(e, asyncssh.misc.PermissionDenied):
                    self._retry -= 1
                self.current_exception = e
                self.logger.error("Unable to create persistent SSH session" f" for {self.hostname} due to {str(e)}")
    ...
    async def _parse_init_dev_data_devtype(self, output, cb_token) -> None:
        ...
        if output[0]["status"] == 0:
            ...
            if timestr:
                self.bootupTimestamp = int(datetime.utcfromtimestamp(parse(timestr.group(1)).timestamp()).timestamp() * 1000)
            else:
                self.logger.error(f"Cannot parse uptime from {self.address}:{self.port}")
        ...
        if (len(output) > 1) and (output[1]["status"] == 0):
            ...
            if hostname:
                ...
                self.logger.error(f"set hostname of {self.address}:{self.port}" f" to {hostname.group(1)}")
...
class IosXENode(Node):
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        """Fill in the boot time of the node by executing certain cmds"""
        await self._exec_cmd(self._parse_init_dev_data, ["show version"], None, "text", reconnect=reconnect)
    ...
    async def _ssh_connect(self):
        ...
        if self.is_connected and not self._stdin:
            self.logger.info(f"Trying to create Persistent SSH for {self.hostname}")
            async with self._cmd_pacer.wait(self.per_cmd_auth):
                try:
                    self._stdin, self._stdout, self._stderr = await self._conn.open_session(term_type="xterm")
                    self.logger.info(f"Persistent SSH created for {self.hostname}")
    ...
    async def _handle_privilege_escalation(self) -> int:
        ...
        self.logger.info(f"Privilege escalation required for {self.hostname}")
        ...
        output = await self.wait_for_prompt()
        if output in ["suzieq timeout", "Password:"] or output.strip().endswith(">"):
            self.logger.error(f"Privilege escalation failed for {self.hostname}" ", Aborting connection")
            return -1

        self.logger.info(f"Privilege escalation succeeded for {self.hostname}")
        return 0

    async def wait_for_prompt(self, prompt: str = None, timeout: int = 90) -> str:
        ...
        try:
            ...
            self.logger.error(f"{self.address}.{self.port} " "Timed out waiting for expected prompt")
    ...
    async def _parse_init_dev_data_devtype(self, output, cb_token) -> None:
        ...
        if output[0]["status"] == 0:
            ...
            if hostupstr:
                ...
                self.bootupTimestamp = int(datetime.utcfromtimestamp(parse(timestr).timestamp()).timestamp() * 1000)
            else:
                self.logger.error(f"Cannot parse uptime from {self.address}:{self.port}")
                self.bootupTimestamp = -1
    ...
    async def _ssh_gather(self, service_callback, cmd_list, cb_token, oformat, timeout, reconnect, only_one=False):
        ...
        if not self.is_connected or not self._stdin:
            if reconnect:
                ...
                logger.debug(f"{self.address}:{self.port} is connecting, " "avoid a nested connection attempt.")

        if not self._conn or not self._stdin:
            for cmd in cmd_list:
                self.logger.error("Unable to connect to node %s (%s) cmd %s", self.address, self.hostname, cmd)
                result.append(self._create_error(cmd))
            await service_callback(result, cb_token)
            return
        ...
        async with self._cmd_pacer.wait(self.per_cmd_auth):
            for cmd in cmd_list:
                try:
                    ...
                    result.append(self._create_error(cmd))
                    if not isinstance(e, asyncio.TimeoutError):
                        self.logger.error(f"Unable to connect to {self.hostname} for {cmd} " f"due to {e}")
                        try:
                            await self._close_connection()
                            self.logger.debug("Closed conn successfully for " f"{self.hostname}")
                        except Exception as close_exc:
                            self.logger.error(f"Caught an exception closing {self.hostname}" f" for {cmd}: {close_exc}")
                    else:
                        self.logger.error(f"Unable to connect to {self.hostname} {cmd} " "due to timeout")
                    break
...
class JunosNode(Node):
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        ...
        await self._exec_cmd(self._parse_init_dev_data, ["show system uptime|display json", "show version"], None, "mixed", reconnect=reconnect)
    ...
    async def _parse_init_dev_data_devtype(self, output, cb_token) -> None:
        ...
        if output[0]["status"] == 0:
            ...
            try:
                ...
                self.logger.warning(f"Unable to parse junos boot time from {data}")
                timestr = '{"junos:seconds": "0"}'
            self.bootupTimestamp = get_timestamp_from_junos_time(timestr, output[0]["timestamp"] / 1000) / 1000
...
class NxosNode(Node):
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        """Fill in the boot time of the node by running requisite cmd"""
        await self._exec_cmd(self._parse_init_dev_data, ["show version", "show hostname"], None, "mixed", reconnect=reconnect)
...
class FWSMNode(IOSNode):
    ...
    async def _fetch_init_dev_data(self):
        ...
        # Set the terminal length to 0 to avoid paging
        await self._exec_cmd(self._null_callback, ["terminal pager 0"], None, "text")

        await self._exec_cmd(self._parse_init_dev_data, ["show version", "show context"], None, "text")
    ...
    def _extract_nos_version(self, data: str) -> None:
        if match := re.search(r"Firewall Version\s+(\S+)", data):
            ...
            self.logger.warning(f"Cannot parse FWSM/ASA version from {self.address}:{self.port} \n {data}")
            self.version = "all"
...
class CPGaia(IOSNode):
    ...
    async def _init_ssh(self, init_dev_data=True, use_lock: bool = True, waitfor=r".*[>#]\s*$", check_privledge=True) -> None:
        ...
        await super()._init_ssh(init_dev_data=False, use_lock=False, check_privledge=False)
        return
    ...
    async def _fetch_init_dev_data(self):
        ...
        # set clienv rows 0
        # Set the terminal length to 0 to avoid paging
        await self._exec_cmd(self._null_callback, ["set clienv rows 0"], None, "text")
        await self._exec_cmd(self._parse_init_dev_data, ["show version product", "show hostname", "show uptime"], None, "text")
...
class SonicNode(Node):
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        """Fill in the boot time of the node by running requisite cmd"""
        await self._exec_cmd(self._parse_init_dev_data, ["cat /proc/uptime", "hostname", "show version"], None, "text", reconnect=reconnect)
...
class PanosNode(Node):
    ...
    async def _fetch_init_dev_data_devtype(self, reconnect: bool):
        ...
        try:
            ...
            async with self._cmd_pacer.wait():
                async with asyncssh.connect(self.address, port=22, username=self.username, password=self.password, known_hosts=None) as conn:
                    async with conn.create_process() as process:
                        process.stdin.write(f"{discovery_cmd}\n")
                        output = ""
                        output += await process.stdout.read(1)
                        try:
                            await asyncio.wait_for(process.wait_closed(), timeout=0.1)
                        except asyncio.TimeoutError:
                            pass

                        stdout, _ = process.collect_output()
                        output += stdout
                        res = [{"status": 0, "data": output}]
            ...
            self.logger.error(f"{self.address}:{self.port}: permission denied")
            self._retry -= 1
        except Exception as e:
            self.logger.error(f'{self.hostname}:{self.port}: Command "{discovery_cmd}" ' f"failed due to {e}")

    async def get_api_key(self):
        """Authenticate to get the api key needed in all cmd requests"""
        url = f"https://{self.address}:{self.port}/api/?type=keygen&user=" f"{self.username}&password={self.password}"

        if not self._retry:
            return
        async with self._cmd_pacer.wait(self.per_cmd_auth):
            async with self._session.get(url, timeout=self.connect_timeout) as response:
                ...
                if status == 200:
                    ...
                    self.logger.error("Invalid credentials, could not get api " f"key for {self.address}:{self.port}.")
                    self._retry -= 1
                else:
                    self.logger.error("Unknown error, could not get " "api key for " f"{self.address}:{self.port}.")
    ...
    async def _parse_init_dev_data_devtype(self, output, cb_token) -> None:
        ...
        if output[0]["status"] == 0:
            data = output[0]["data"]
            # extract uptime
            match = re.search(r"uptime:\s+(\d+)\sdays,\s(\d+):(\d+):(\d+)", data)
            ...
            if match:
                ...
                upsecs = 86400 * int(days) + 3600 * int(hours) + 60 * int(minutes) + int(seconds)
                self.bootupTimestamp = int(int(time.time() * 1000) - float(upsecs) * 1000)
            else:
                self.logger.warning(f"Cannot parse uptime from {self.address}:{self.port}")
            ...
            if hmatch:
                ...
                self.logger.warning(f"Cannot parse hostname from {self.address}:{self.port}")
        ...
        if not self._session:
            async with self._cmd_pacer.wait(self.per_cmd_auth):
                try:
                    ...
                    self.logger.error(f"{self.transport}://{self.hostname}:{self.port}, " f"Unable to communicate due to error: {str(e)}")
        ...
        async with self._cmd_pacer.wait(self.per_cmd_auth):
            try:
                for cmd in cmd_list:
                    ...
                    async with self._session.get(url_cmd, timeout=timeout) as response:
                        status, xml = response.status, await response.text()
                        if status == 200:
                            json_out = json.dumps(xmltodict.parse(xml))
                            result.append(
                                {
                                    "status": status,
                                    "timestamp": now,
                            ...
                            result.append(self._create_error(cmd))
                            self.logger.error(
                                f"{self.transport}://{self.hostname}:" f"{self.port}: Command {cmd} failed with " f"status {response.status}"
                            )
            except Exception as e:
                self.current_exception = e
                for cmd in cmd_list:
                    result.append(self._create_error(cmd))
                self.logger.error(f"{self.transport}://{self.hostname}:{self.port} " f"Unable to communicate due to {str(e)}")
