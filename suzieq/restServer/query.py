...
def query_network_depr_summarize(
    request: Request,
    token: str = Depends(get_api_key),
    format: str = None,
    columns: List[str] = Query(default=["default"]),
...
def read_shared(function_name, verb, request, local_variables=None):
    ...
    command_args, verb_args = create_filters(
        function_name, command, request, local_variables
    )
...
def run_command_verb(command, verb, command_args, verb_args, columns=None, format=None):
    """
    ...
    svc = get_svc(command)
    try:
        svc_inst = svc(**command_args, config_file=app.cfg_file, engine_name="pandas")
        df = getattr(svc_inst, verb)(**verb_args)

    except AttributeError as err:
        return_error(404, (f"{err}"))
...
def missing_verb(command):
    return_error(
        404,
        f"{command} command missing a verb. for example " f"/api/v2/{command}/show",
    )
