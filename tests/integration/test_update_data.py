...
def run_scenario(scenario):
    ...
    run_cmd(["ansible-playbook", "-b", "-e", f"scenario={scenario}", "deploy.yml"])
    time.sleep(10)
...
class TestUpdate:
    ...
    @pytest.mark.skipif(
        not os.environ.get("SUZIEQ_POLLER", None), reason="Not gathering data"
    )
    ...
    def test_gather_cumulus_data(self, tmp_path, vagrant_setup):
        ...
        gather_data(
            "dual-attach", "evpn", "centralized", "dual-evpn", orig_dir, tmp_path
        )
        update_input_data(orig_dir, "cumulus", "dual-evpn", tmp_path)

        gather_data(
            "single-attach", "ospf", "numbered", "ospf-single", orig_dir, tmp_path
        )
    ...
    @pytest.mark.cumulus
    @pytest.mark.skipif(
        not os.environ.get("SUZIEQ_POLLER", None), reason="Not updating data"
    )
    ...
    def _update_test_data_common_fn(self, nos, tmp_path, device_cnt):
        ...
        update_data(
            nos,
            f"{orig_dir}/tests/integration/sqcmds/{nos}-input/",
            orig_dir,
            tmp_path,
            number_of_devices=device_cnt,
        )
    ...
    def test_update_multicast(self, tmp_path):
        """Update test data for multicast."""
        self._update_test_data_common_fn("multicast", tmp_path, "6")
...
def _update_cndcn_data(topology, proto, scenario, tmp_path):
    ...
    update_data(
        name, f"{orig_dir}/tests/integration/all_cndcn/{name}-input", orig_dir, tmp_path
    )
    ...
    if os.environ.get("UPDATE_SQCMDS", None):
        update_sqcmds(
            glob.glob(f"{cndcn_samples_dir}/{name}-samples/*.yml"),
...
def _test_data(topology, proto, scenario, testvar):
    ...
    dummy_config = load_sq_config(config_file=conftest.create_dummy_config_file())
...
class TestDualAttach:
    ...
    @pytest.mark.skipif("SUZIEQ_POLLER" not in os.environ, reason="Not updating data")
...
@pytest.mark.cleanup
@pytest.mark.skipif("SUZIEQ_POLLER" not in os.environ, reason="not sqpoller")
def test_cleanup_vagrant():
    """Cleanup sim"""
    devices = [
        "dual-attach_internet",
        "dual-attach_spine01",
        "dual-attach_spine02",
        "dual-attach_leaf01",
        "dual-attach_leaf02",
        ...
        "dual-attach_exit01",
        "dual-attach_exit02",
        "dual-attach_server101",
        "dual-attach_server102",
        "dual-attach_server103",
        "dual-attach_server104",
        "dual-attach_edge01",
        "single-attach_internet",
        "single-attach_spine01",
        "single-attach_spine02",
        "single-attach_leaf01",
        "single-attach_leaf02",
        "single-attach_leaf03",
        "single-attach_leaf04",
        "single-attach_exit01",
        "single-attach_exit02",
        "single-attach_server101",
        "single-attach_server102",
        "single-attach_server103",
        "single-attach_server104",
        "single-attach_edge01",
    ]
