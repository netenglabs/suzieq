...
command_table = [
    ...
    ['ospf', basic_verbs + ['assert', 'top'],
     [[None, '--columns=hostname','--type=interface'],
      [None, '--namespace=dual-evpn', '--type=interface'],
      '--columns=hostname',
      None,
      None]],
    ['path', ['show', 'summarize'],
     [['--dest=172.16.2.104 --src=172.16.1.101 --namespace=dual-evpn',
       '--dest=172.16.2.104 --src=172.16.1.104 --namespace=dual-evpn',
       '--dest=10.0.0.11 --src=10.0.0.14 --namespace=ospf-single',
       '--src=172.16.1.101 --dest=172.16.253.1 --namespace=dual-evpn'],
      ['--dest=172.16.2.104 --src=172.16.1.101 --namespace=dual-evpn']]],
